...
<template>
    ...
    <div class="border border-stone-500 shadow-xl rounded-lg w-full h-full flex flex-col bg-white dark:bg-gray-800 dark:border-gray-700 overflow-hidden group transition-colors duration-300">
      <router-link :to="{name:'productDetails', params:{id: productDetails._id}}">
        <div class="relative">
          <img :src="productDetails.image" class="w-full h-40 sm:h-60 rounded-lg group-hover:scale-110 object-cover transition-transform duration-500" />
        ...
        <div class="px-4 pt-5">
          <div class="flex justify-between items-center gap-1">
            <p class="text-stone-500 dark:text-gray-400 text-[8px] sm:text-[10px] truncate max-w-[60%]">{{ productDetails.category }}</p>
            <p class="text-stone-500 dark:text-gray-400 text-[10px] sm:text-[12px] whitespace-nowrap">⭐ {{ productDetails.rating }}</p>
          </div>
          <h1 class="font-semibold text-[#2e251f] dark:text-gray-200 text-xs sm:text-base leading-tight mt-1 truncate">{{ productDetails.title }}</h1>
          <p class="text-gray-600 dark:text-gray-400 my-1 font-medium text-xs sm:text-base">₹{{ ((Number(productDetails.pricePaise) / 100) || Number(productDetails.price || 0)).toFixed(2) }}    <span class="text-[10px] sm:text-[12px] line-through text-gray-400 dark:text-gray-600 whitespace-nowrap ml-1">₹{{ (((Number(productDetails.pricePaise) / 100) || Number(productDetails.price || 0))*1.33).toFixed(2) }}</span></p>
        </div>
      </router-link>
      <div class="text-center pb-5 px-2">
